  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 transition-colors duration-200 shadow-lg rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-800">
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{d.name}</p>
        <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500">{formatINR(d.value)} ({pct}%)</p>
      </div>
    );
  }
  return null;
      ...
      <ResponsiveContainer width="100%" height={280}>
        <PieChart>
          ...
          <Legend
            ...
            height={36}
            formatter={(value) => <span className="text-xs text-gray-600 dark:text-gray-400 dark:text-gray-500">{value}</span>}
          />
        </PieChart>
      </ResponsiveContainer>
    </div>
