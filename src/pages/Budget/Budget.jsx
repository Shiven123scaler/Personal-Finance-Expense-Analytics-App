  ...
  return (
    ...
    >
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-1">{formatMonth(currentMonth)}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Category breakdown */}
          <div className="bg-white dark:bg-gray-800 transition-colors duration-200 rounded-xl p-6 border border-gray-100 dark:border-gray-800">
          ...
          <div className="bg-white dark:bg-gray-800 transition-colors duration-200 rounded-xl p-6 border border-gray-100 dark:border-gray-800">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Recurring Expenses</h3>
            </div>
            ...
            {recurringExpenses.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500">No recurring expenses this month.</p>
            ) : (
                ...
                {recurringExpenses.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between py-2 border-b border-gray-50 dark:border-gray-800 last:border-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-300">{tx.title}</span>
                    </div>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatINR(tx.amount)}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {!isEditing && (
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-[#185FA5] hover:underline cursor-pointer"
                >
            ...
            {isEditing ? (
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {EXPENSE_CATEGORIES.filter((c) => c !== 'Other').map((cat) => (
                      <div key={cat} className="flex items-center gap-3">
                        <span className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500 w-28">{cat}</span>
                        <input
                          type="number"
                          ...
                          className="flex-1 px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#185FA5]/20"
                          placeholder="₹0"
                        />
                      </div>
                    ))}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/30 dark:bg-gray-900 transition-colors duration-200 transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                  <motion.button
                    onClick={handleSave}
                    whileTap={{ scale: 0.98 }}
                    className="flex-1 py-2.5 bg-[#185FA5] text-white rounded-xl text-sm font-semibold hover:bg-[#0C447C] transition-colors cursor-pointer"
                  >
                    Save Budget
                  </motion.button>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-gray-50 dark:border-gray-800">
                  <span className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">Monthly Total</span>
                ...
                {Object.entries(budget.categories).map(([cat, amount]) => (
                  <div key={cat} className="flex justify-between py-1.5">
                    <span className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{cat}</span>
                    <span className="text-sm text-gray-700 dark:text-gray-300">{formatINR(amount)}</span>
                  </div>
                ))}
              </div>
            )}
